...
import beveragesData from '@/data/mock/beers.json'
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetBeverages') || query.includes('nodeBeverages')) {
      return loadMockData('beers.json')
    }
